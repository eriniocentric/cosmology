...
open OUT, ">$ENV{HOME}/Software/massfunction/completeness_evol.dat" or die "can't open zmax file"; 
...
while (<IN>) {
    ...
    $ageatz = $t2($itime)->index(0);
    
#array with edge of zbins
    $zbins = pdl(1.0,1.5,2.0);
...
#get mass and Kobs from Masses file
	...
	if ($zsp < $zbin) {
	    $ageatzbin = ($ageatz - ((lookback($zbin) - lookback($zsp))) * 0.001)->index(0);
	    $itime_evol = which( $ageatzbin < $t2)->index(0);
	}
	else {
	    $ageatzbin = ($ageatz + ((lookback($zsp) - lookback($zbin))) * 0.001)->index(0);
	    $itime_evol = which( $ageatzbin < $t2)->index(0);
	    
	}

	$Ms2 = log10( $Ms($itime_evol));
	$Ms2 = $Ms2->index(0);
	$spec2 = $spec->dice_axis(1,$itime_evol); 
	$emspec2 = $emspec->dice_axis(1,$itime_evol); 
...
# add 3.3 um PAH feature following Lorentian Profile parametrized by Verstraete et al. (2001)
	...
	while ($K < 20.6) {
	    ...
	    ($wz, $sz) = redshift_spectra($wav, $specvary, pdl($zbin));
...
#when $K goes below 20.6, completeness limit for this SED template is reached
	    ...
	    $massmin($i) .= $Ms2 + log10( $fac) + $logfacmass + 0.1 ; #add 0.1 dex to correct for last step
	    
	}
    }
    
    printf OUT "%10s    %5.2f    %5.2f    %5.2f       %5.2f       %5.2f \n", $id,$zsp,$Kobs,$massmin((0)),$massmin((1)),$massmin((2));
}
close IN; close OUT;

($z,$K,$ms1,$ms2,$ms3) = rcols("$ENV{HOME}/Software/massfunction/completeness_evol.dat",1,2,3,4,5);

$ix = which($K <= 20.6);

$limits = zeroes(3);
$limits(0) .=  qsort( $ms1($ix))->(0.5 * nelem($ix));
$limits(1) .=  qsort( $ms2($ix))->(0.5 * nelem($ix));
$limits(2) .=  qsort( $ms3($ix))->(0.5 * nelem($ix));
