...
#when $K goes below 20.6, completeness limit for this SED template is reached
    ...
    $massmin4 = $Ms2 + log10( $fac) + $logfacmass + 0.1; #add 0.5 dex to correct for last step

    printf OUT "%10s    %5.2f    %5.2f    %5.2f       %5.2f       %5.2f    %5.2f \n", $id,$zsp,$Kobs,$massmin1,$massmin2,$massmin3,$massmin4;
}
close IN; close OUT;

($z,$K,$ms1,$ms2,$ms3,$ms4) = rcols("$ENV{HOME}/Software/massfunction/completeness.dat",1,2,3,4,5,6);
...
$limits(3) .=  qsort( $ms4($ix))->(0.5 * nelem($ix));
